...
export async function retrieveJobs(jobType,location,searchField,distanceFilter){
    ...
    const link = `/api/v1/api/jobs/us/search/1?app_id=a93ec7ad&app_key={api_key}&results_per_page=20&where=${location}&distance=${distanceFilter}&title_only=${searchContent}`;
    let job=""
    if(jobType!==null){
        job = `&${jobType}=1`
    }
