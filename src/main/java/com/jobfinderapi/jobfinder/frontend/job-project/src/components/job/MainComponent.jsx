...
export default function MainComponent(){
    ...
    const {jobType,isLoading,
        location,searchField,distanceFilter,currentItem,setCurrentItem,setTimeEst,timeEst} = useContext(AppContext);
    ...
    useEffect(()=>{
        async function fetchData(){
            ...
            const collection = await retrieveJobs(jobType,location,searchField,distance,timeEst);
    ...
    return(
        <div className="main-body">
            ...
            <div className="top-content-container">
                ...
                {allowMap &&(currentItem!==null) ? <Map latitude={lat} longitude={long} destLat={currentItem.latitude} destLong={currentItem.longitude}/> : 
                (collection?.length>0)&&(currentItem!==null)&&<div id="map">
                    <button className="map-button"onClick={generateMapFromCurrent}>Generate map from current location</button>
                </div>
                }   
            </div>
            ...
            <div className="sidebar">
                {collection?.length>0 ? <Item collection={collection} changeTopBox={changeTopBox} changeBottomBox={changeBottomBox}/>: <p>no results found</p> }
            </div>
        </div>
    )
}